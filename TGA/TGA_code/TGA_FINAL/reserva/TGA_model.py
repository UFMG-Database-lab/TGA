...
class TGA(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        self.nclass= len(self.le.classes_)
        ...
        data_loader_val = DataLoader(list(zip(Gs_val, y_val)), batch_size=self.transform_batch_size,
        ...
        for epoch in range(self.n_epochs):
            ...
            with tqdm(total=len(data_loader_val.dataset), smoothing=0., disable=not self.verbose) as pbar:
                ...
                score = correct/total
                if self.verbose:
                    self.checkpoints.append( (score_train, score) )
    ...
    def transform(self, X):
        pass
    def predict(self, X):
        Gs_test   = self.graph_builder.transform( X )
        data_loader_val = DataLoader(Gs_test, batch_size=self.transform_batch_size,
                                     shuffle=False, collate_fn=self.collate_test)
        results = []
        with tqdm(total=len(data_loader_val.dataset), smoothing=0., disable=not self.verbose) as pbar:
            self.model.eval()
            for bg in data_loader_val:
                with torch.no_grad():
                    pred = self.model.predict(bg)
                    pred = self.le.inverse_transform(list(pred.cpu().numpy()))
                    results.extend( pred )
                    pbar.update( bg.batch_size )
        return results